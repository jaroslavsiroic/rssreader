...
function init() {
    ...
    $(document).on("pageshow", "#feedpage", function(e) {
        ...
        if (!feedCache[thisFeed.url]) {
            ...
            $.get("https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=10&q=" + encodeURI(thisFeed.url) + "&callback=?", {},
                function(res, code) {
...
function displayFeeds() {
    ...
    if (feeds.length == 0) {
        ...
        for (var i = 0; i < feeds.length; i++) {
            s += "<li><a href='feed.html?id=" + i + "' data-feed='" + i + "'>" + feeds[i].name + "</a> <a href='' class='deleteFeed' data-feed-id='" + i + "' >Delete</a></li>";
        }
        $("#feedList").html(s);
